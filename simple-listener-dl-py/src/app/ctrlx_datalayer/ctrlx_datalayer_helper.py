"""
Helper script to connect to ctrlX Data Layer.

This script provides auxiliary methods to create ctrlX Datalayer client and
provider connections to ctrlX CORE devices.

It can be used for both running in an app build environment (QEMU VM) and
within the snap environment on the ctrlX CORE.

Feel free to use it in your projects and to change it if necessary.

For ease of use, the default values for IP address, user, password and SSL
port are chosen to match the settings of a newly created ctrlX CORE device:
...
If these values do not suit your use case, explicitly pass the parameters that
...
1. ctrlX CORE or ctrlX COREvirtual with another IP address, user and password:
    ...
    client, client_connection = get_client(system, ip='192.168.1.100',
                                user='admin', password='-$_U/{X$aG}Z3/e<')

2. ctrlX COREvirtual with port forwarding running on the same host as the app
   build environment (QEMU VM):

    client, client_connection =
        get_client(system, ip='10.0.2.2', ssl_port=8443)

    Remarks:
    10.0.2.2 is the IP address of the host from the point of view of the app
    build environment (QEMU VM).
    8443 is the host port which is forwarded to the SSL port (=433) of the
    ctrlX COREvirtual


IMPORTANT:
You need not change the parameter settings before building a snap and
installing the snap on a ctrlX CORE.
The method get_connection_string detects the snap environment and uses
automatically inter process communication.
...
def get_connection_string(
        ...
        ssl_port=443):
    """
    Combine a ctrlX Datalayer connection string.
    ...
    @param[in] ssl_port Port number for a SSL connection. ctrlX COREvirtual with port forwarding: Use the host port (default 8443) forwarded to port 22 of the ctrlX virtual.
...
def get_client(system: ctrlxdatalayer.system.System,
               ...
               ssl_port=8443):
    """
    Create a ctrlX Datalayer client instance.
...
def get_provider(system: ctrlxdatalayer.system.System,
                 ...
                 ssl_port=443):
    """
    Create a ctrlX Datalayer provider instance.
